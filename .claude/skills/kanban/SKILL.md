...
6. **Plan 质量门禁 (如果 enabled):**
   ...
   ```
   - 对4个维度评分: 需求清晰度、技术可行性、任务拆解合理性、验收标准完整性
   - 总分 >= pass_threshold (默认 9.0) 通过
   - 不通过时自动重试 Plan (最多 max_rounds 轮，默认 3)
   - 超过 max_rounds 仍不达标 -> 进入 user_decision，提示用户选择
   - 评分记录到 task JSON 的 `plan_quality` 字段
7. `workflow_complete_phase "$task_id"`
...
5. **按 subtask 逐步调度 executor Agent:**
   ...
   - 每个 subtask 的执行流程:
     ...
     b. 调度 **executor Agent**:
        ...
        请执行此子任务:
        1. 读取 {report_dir}/requirements.md 了解整体需求
...
2. `evaluator_prepare_all "$task_id"` -- 准备 4 个角色的调度上下文
3. **并行启动 4 个评估 Agent:**

   对每个角色 (code_reviewer, qa, pm, designer) 启动独立 Agent:
...
5. 收集评分:
   ...
   ```
6. `evaluator_collect_scores "$task_id"` -- 展示评分
7. `workflow_complete_phase "$task_id"`
8. 进入自迭代判断
...
2. `kanban_iteration_summary "$task_id"` -- 展示迭代摘要（评分趋势、扣分项）
3. `kanban_changes_summary "$task_id"` -- 展示变更摘要（提交记录、文件变更、关键改动、产物文件、评估报告链接）
4. 等待用户执行 `/kanban decide`:
   - `approve_and_archive` -> 合并 worktree + 归档
   - `restart_from_plan` -> 重置到 Plan 阶段
   - `restart_from_execute` -> 重置到 Execute 阶段
   - `abort` -> 归档任务

#### Phase 4.5: Retrospective

**触发条件:** Evaluate 完成后，如果 workflow.json 包含 retrospective 阶段
...
4 个评估 Agent 并行启动 (run_in_background=true)，每个使用原生 agent 名称:

```
Agent(
  subagent_type="{role}",        # code-reviewer / qa / pm / designer
