    ...
    if [ "$quality_gate_enabled" = "true" ]; then
      ...
      if [ $quality_exit -ne 0 ] || [ ! -f "$quality_json_file" ]; then
        ...
        _atomic_jq_write "$tf" \
          ...
          '.plan_quality = {last_score: $score, issues: $issues, retries: $retries, max_rounds: $max_rounds}'
        ...
        _atomic_jq_write "$tf" --argjson score "$quality_score" \
          '.plan_quality = {last_score: $score, retries: (.plan_quality.retries // 0), pass: true}'
...
workflow_start_iteration() {
  ...
  _atomic_jq_write "$tf" \
    ...
    '.iteration=$iter | .updated_at=$now | .history += [{"phase":"self_improve", "iteration":$iter, "type":$type, "entered_at":$now}]'
