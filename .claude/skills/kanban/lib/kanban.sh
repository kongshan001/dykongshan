...
kanban_create_task() {
  ...
  if [ -f "$idx" ]; then
    local _itmp=$(mktemp)
    jq --argjson _s "{id:\"$task_id\",status:\"pending\",phase:null,iteration:0}" \
      '.tasks += [$_s]' "$idx" > "$_itmp" && mv "$_itmp" "$idx"
  fi
...
kanban_show_task() {
  ...
  if [ "$has_scores" -gt 0 ] 2>/dev/null; then
    ...
    for role in code_reviewer qa pm designer; do
      local score=$(jq -r ".scores.${role}.score // \"\"" "$tf")
      local passed=$(jq -r ".scores.${role}.passed // \"\"" "$tf")
      [ -z "$score" ] && continue
      local pass_icon=""
...
kanban_score_history() {
  ...
  echo ""
  echo "Iteration | code_reviewer | qa     | pm     | designer | Avg"
  echo "----------|---------------|--------|--------|----------|-----"
