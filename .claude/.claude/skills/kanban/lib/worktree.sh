...
# ST-002: 同步运行时配置文件到 worktree 目录
# 从主仓库的 .kanban/ 复制 workflow.json 和 config.json 到 worktree 的 .kanban/ 目录
# 幂等: 目标文件已存在时不覆盖
# 优先从主仓库 .kanban/ 复制实际运行时配置；如果不存在，回退到模板目录
_worktree_sync_config() {
  local wt_dir="$1"
  local wt_kanban_dir="$wt_dir/$KANBAN_DIR"

  # 确保 worktree 的 .kanban/ 目录存在
  mkdir -p "$wt_kanban_dir"

  # 定位主仓库的 .kanban/ 目录
  # 策略: git rev-parse --show-toplevel 返回 worktree 对应的主仓库绝对路径
  # 对于 git worktree 链接目录，这指向主仓库；对普通子目录也能正确解析
  local main_repo_dir
  main_repo_dir="$(git -C "$wt_dir" rev-parse --show-toplevel 2>/dev/null)"
  # 回退: 如果 show-toplevel 失败，尝试从 git-common-dir 推算
  if [ -z "$main_repo_dir" ] || [ ! -d "$main_repo_dir" ]; then
    local git_common_dir
    git_common_dir="$(git -C "$wt_dir" rev-parse --git-common-dir 2>/dev/null)"
    if [ -n "$git_common_dir" ]; then
      # 将相对路径转为绝对路径（cd 到 wt_dir 再 resolve）
      main_repo_dir="$(cd "$wt_dir" && cd "$(dirname "$git_common_dir")" 2>/dev/null && pwd)" || main_repo_dir=""
    fi
  fi
  ...
  local main_kanban_dir="$main_repo_dir/$KANBAN_DIR"

  # 复制 workflow.json: 优先从主仓库运行时目录，回退到模板
  if [ ! -f "$wt_kanban_dir/workflow.json" ]; then
    if [ -f "$main_kanban_dir/workflow.json" ]; then
      cp "$main_kanban_dir/workflow.json" "$wt_kanban_dir/workflow.json"
      echo "Synced workflow.json to worktree (from main repo)"
    elif [ -n "${SKILL_DIR:-}" ] && [ -f "$SKILL_DIR/templates/workflow.json" ]; then
      cp "$SKILL_DIR/templates/workflow.json" "$wt_kanban_dir/workflow.json"
      echo "Synced workflow.json to worktree (from template)"
    else
      echo "WARNING: No workflow.json source found for worktree config sync"
    fi
  fi

  # 复制 config.json: 优先从主仓库运行时目录，回退到模板
  if [ ! -f "$wt_kanban_dir/config.json" ]; then
    if [ -f "$main_kanban_dir/config.json" ]; then
      cp "$main_kanban_dir/config.json" "$wt_kanban_dir/config.json"
      echo "Synced config.json to worktree (from main repo)"
    elif [ -n "${SKILL_DIR:-}" ] && [ -f "$SKILL_DIR/templates/config.json" ]; then
      cp "$SKILL_DIR/templates/config.json" "$wt_kanban_dir/config.json"
      echo "Synced config.json to worktree (from template)"
    else
      echo "WARNING: No config.json source found for worktree config sync"
    fi
  fi
}
...
worktree_create() {
  ...
  if git worktree add "$wt_dir" -b "$branch" HEAD 2>/dev/null; then
    ...
    # ST-002: 自动复制运行时配置文件到 worktree 目录
