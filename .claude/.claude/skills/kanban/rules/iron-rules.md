...
本文件包含 kanban 多 Agent 编排系统的 17 条 Iron Rules (铁律) 的详细定义。
...
- **执行方式**: 知识提取在归档时由 `_extract_knowledge_on_archive()` 自动执行。该函数采用 best-effort 策略：尽力从 pitfalls 和 decisions 文件中提取知识，但提取失败（文件不存在或内容为空）时不阻塞归档，仅记录警告日志。retrospective 阶段的 Knowledge Manager 仍应尽力产出至少1条新知识条目。
- **违反后果**: retrospective 阶段 Knowledge Manager 应产出至少1条新知识条目。归档时自动提取如失败仅记录警告，不阻塞归档流程。
...
- **执行方式**: 此规则通过 Agent 编排器约定执行（kanban skill 的 Archive 阶段在 commit 后自动 push），而非 Guard 层强制检查。编排器在提交代码后必须立即执行 push。
