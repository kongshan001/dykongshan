...
## 检查方法
`Guard.check_artifacts()` 在各阶段转换时验证产物完整性:

1. Plan -> Plan Review 转换时，检查 `requirements.md` 和 `task_breakdown.json` 存在且非空
2. Execute -> Evaluate 转换时，检查 `execution_summary.md`、`execution_pitfalls.md` 和 `execution_decisions.md` 存在且非空
3. Evaluate -> Retrospective 转换时，检查 4 个角色报告 JSON 文件存在（通过 `check_evaluation` 验证）及 `acceptance.md`
...
**注意:** 当前版本不做内容格式校验（如 JSON 解析正确性、Markdown 标题结构）。这些由下游消费者（评估 agent、plan reviewer 等）在读取时自行验证。

## 违反后果
Guard 检测到产物缺失时，阻止阶段转换。系统输出明确的错误信息，指出缺失的文件名和当前阶段。相关 agent（planner / executor / evaluator）必须补全缺失的文档产物后才能继续流程。
